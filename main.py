...
class Gun(pygame.sprite.Sprite):
    ...
    def update(self, *args):
        ...
        if enemy_bullets and pygame.sprite.collide_rect(self, enemy_bullets[0]):
            ...
            if self.life == 0:
                update_bd(user_name, level_num, all_score)
                death_screen()

        if pygame.sprite.spritecollideany(self,
                                          enemies):  # при столкновении пришельца и пушки он умирает и возраждается через 2 секунды
            time.sleep(2)
            main()
...
class Enemy(pygame.sprite.Sprite):
    ...
    def update(self):
        """
        передвижение и смерть пришельцев
        """
        global all_score
        self.move_counter += 1
        ...
        for i in range(len(all_aliens)):  # проверка на прилепания пришельцев друг к другу
        ...
        if bullets and pygame.sprite.collide_rect(self,
                                                  bullets[0]):  # смерть пришельца от пули игрока
            bullets[0].kill()
            bullets.pop(0)
...
class Enemy_bullets(pygame.sprite.Sprite):
    ...
    def update(self):
        ...
        if bullets:
            if pygame.sprite.collide_rect(self, bullets[
                0]):  # при столкновении вражеской пули с пулей игрока, она умирает
                self.kill()
                bullets[0].kill()
                bullets.pop(0)
                enemy_bullets.pop(0)
...
class Octavius(pygame.sprite.Sprite):
    ...
    def update(self):
        ...
        for i in range(len(all_aliens)):  # проверка на прилепание осьминогов друг к другу
            for j in range(len(all_aliens[i]) - 1):
                if not isinstance(all_aliens[i][j], str):
                    c = 1
                    while j + c < len(all_aliens):
...
def collide_en_bul_with_obst(en_bul, bars):
    """
    разрушение преград от выстрелов пришельцев
    :param en_bul:
    :param bars:
    :return:
    """
...
def collide_bul_with_obst(bul, bars):
    """
    разрушение преград от выстрелов пушки
    :param bul:
    :param bars:
    :return:
    """
...
class Block(pygame.sprite.Sprite):
    """
    создание преград, рушущиеся от выстрелов и атак пришельцев
    """
...
def update_bd(user_name: str, level_num: str, all_score: int) -> None:
    ...
    if get_levels(user_name)[0] == int(level_num) and int(level_num) < 5:
        ln = int(level_num) + 1
    else:
